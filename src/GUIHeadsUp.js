...
this.pfxAccumTime = 0;
this.flashTimes = [];
for(var i = 0; i < this.capacity; ++ i) this.flashTimes.push(0);
...
this.flashTimes = [];
for(var i = 0; i < cap; i ++) this.flashTimes.push(0);
this.capacity = cap;
this.hp = h;
};
...
this.cacheNeedsUpdate = true;
if(c > this.capacity){
for(var i = this.flashTimes.length; i < c; i ++) this.flashTimes.push(0);
}else if(c < this.capacity){
for(var j = 0; j < this.flashTimes.length - c; j ++) this.flashTimes.pop();
}
this.capacity = c;
};
...
this.cacheNeedsUpdate = true;
if(h > this.hp){
for(var i = this.hp; i < h; i ++) this.flashTimes[i] = - this.kFlashTime;
}else if(h < this.hp){
for(var j = Math.min(this.capacity - 1,this.hp - 1); j >= h; j --) this.flashTimes[j] = this.kFlashTime;
}
this.hp = h;
};
...
if(end < start){
var cnt = 0;
for(; angles[index] > end && cnt < 3; cnt ++, index = (index + 1) % 4) this.vertices[currVert ++] = new Vertex(corners[index][0],corners[index][1],corners[index][2],corners[index][3]);
}else{
for(; angles[index] < end && index < 4; index ++) this.vertices[currVert ++] = new Vertex(corners[index][0],corners[index][1],corners[index][2],corners[index][3]);
}
...
kAcorn:0,kMultiplier:1,kStick:2,kHealth:3
};
